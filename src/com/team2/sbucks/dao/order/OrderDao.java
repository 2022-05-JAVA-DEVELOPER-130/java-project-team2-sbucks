...
public class OrderDao {
	...
	public OrderList selectOrder(int member_no,int order_no) throws Exception{
		...
		if(rs.next()) {
			...
			selectOrder=new OrderList(rs.getInt("order_no"), rs.getDate("order_date"),rs.getInt("member_no"), rs.getInt("order_price"));
	...
	public List<OrderList> selectAll(int member_no) throws Exception {
		...
		ResultSet rs=pstmt.executeQuery();
		while(rs.next()) {
			orderList.add(new OrderList(rs.getInt("order_no"), rs.getDate("order_date"), rs.getInt("member_no"), rs.getInt("order_price")));
		}
